...
def convert_pth_to_toty_gguf(pth_path="bilingual_retro_tiny.pth", gguf_path="toty.gguf"):
    """
    Saves and serializes the weights of our trained Bilingual Tiny Retro Diffusion model
    (trained directly on the user's uploaded image patterns) into GGUF format, mapping all layers
    to standard names. The resulting model is saved as 'toty.gguf' in the root directory.
    This model is lightweight (~80MB) and optimized to run locally on low-end Android CPUs
    without requiring any GPU/graphics card and with very low RAM consumption.
    """
    print(f"=== Converting '{pth_path}' to optimized 'toty.gguf' ===")

    # Ensure path exists or fallback to loaded weights
    if os.path.exists(pth_path):
        state_dict = torch.load(pth_path, map_location="cpu")
        print(f"Loaded trained user weights successfully from: {pth_path}")
    else:
    ...
    # This prevents crashes inside local-dream engines on low-resource mobile platforms.
    mapping = {
    ...
    print(f"🎉 Success! Generated highly optimized GGUF model: {gguf_path}")
    print(f"File saved successfully at the root directory: {os.path.abspath(gguf_path)}")

if __name__ == "__main__":
    convert_pth_to_toty_gguf()
